...
//                     Text Content
//                 </label>
//                 <textarea value={values.text} onChange={(e)=>handleChange('text',e.target.value)} className='w-full text-sm p-2 border border-gray-500 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none min-h-20'/>
//             </div>
//             <div>
//                 <label className='block text-xs font-medium text-gray-500 mb-1'>
//                     Class Name
...
//                         className='w-6 h-6 rounded cursor-pointer0000 '/>
...
const EditorPanel = ({ selectedElement, onUpdate, onClose }: EditorPanelProps) => {
  ...
  return (
    <div className='absolute top-4 right-4 w-80 bg-white rounded-lg shadow-xl border border-gray-200 p-4 z-50 max-h-[90vh] overflow-y-auto animate-fade-in fade-in '>
      <div className='flex justify-between items-center mb-4'>
        <h3 className='font-semibold text-gray-800'>Edit Element</h3>
        <button onClick={onClose} className='p-1 hover:bg-gray-100 rounded-full'>
          <X className="w-4 h-4 text-gray-500" />
      ...
      <div className='space-y-4 text-black'>
        <div>
          ...
          <textarea
            ...
            className='w-full text-sm p-2 border border-gray-500 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none min-h-20'
          />
        </div>
        ...
        <div className='grid grid-cols-2 gap-3'>
          <div>
            ...
            <div className='flex items-center gap-2 border rounded-md p-1 border-gray-400'>
              <input
                type='color'
                value={values.styles?.backgroundColor === 'rgba(0, 0, 0, 0)' ? '#ffffff' : values.styles?.backgroundColor || '#ffffff'}
                onChange={(e) => handleStyleChange('backgroundColor', e.target.value)}
                className='w-6 h-6 rounded cursor-pointer'
              />
              <span className='text-xs text-gray-600 truncate'>{values.styles?.backgroundColor || 'none'}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className='flex items-center gap-2 border rounded-md p-1 border-gray-400'>
              <input
                ...
                className='w-6 h-6 rounded cursor-pointer'
              />
              <span className='text-xs text-gray-600 truncate'>{values.styles?.color || 'none'}</span>
            </div>
          </div>
        </div>
      </div>
