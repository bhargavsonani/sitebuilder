...
const MyProjects: React.FC = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const fetched = (data?.data?.projects ?? []).map((p: any) => ({
        ...p,
        id: p.id || p._id,
      }));
        ...
        {loading ? (
          ...
          <div className="py-10 min-h-[80vh]">
            <div className="flex justify-between items-center">
              ...
              <button
                onClick={() => navigate("/")}
                className="flex gap-2 items-center bg-indigo-600 px-4 py-2 rounded"
              >
            ...
            <div className="flex flex-wrap gap-4 mt-6">
              {projects.map((project) => (
                <div
                  key={project.id}
                  onClick={() => navigate(`/projects/${project.id}`)}
                  className="w-72 cursor-pointer bg-gray-900 rounded-lg border border-gray-700 overflow-hidden"
                >
